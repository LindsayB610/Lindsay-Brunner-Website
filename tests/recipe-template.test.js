...
function validateRecipeTemplate() {
  ...
  files.forEach(file => {
    ...
    if (hasMethod) {
      // Stop at next ## section or --- separator. Do not use $ in lookahead (with /m, $ matches
      // end of any line, so the non-greedy match would stop after the first line of the method).
